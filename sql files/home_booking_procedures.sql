...
-- 7. procedure to get show and hall for movie
CREATE OR ALTER PROCEDURE GetShowAndHallForMovie
    @MovieID INT,
    @ShowDate DATE,
    @ShowTime TIME
AS
BEGIN
    ...
    SELECT s.showID, h.hall_no
    FROM show s
    JOIN movie m ON s.movieID = m.movieID
    JOIN hall h ON s.hall_no = h.hall_no
    WHERE m.movieID = @MovieID
      AND s.show_date = @ShowDate
      AND CONVERT(TIME, s.show_time) = CONVERT(TIME, @ShowTime);
END
GO
...
-- 11. get ticket price by show id
CREATE OR ALTER PROCEDURE GetBasePriceByShowID
    @ShowID INT
AS
BEGIN
    SET NOCOUNT ON;
